...
-- Adicionando a coluna CREF na tabela professor
ALTER TABLE professor 
ADD COLUMN cref VARCHAR(10) UNIQUE;

--  Função para gerar matrícula
CREATE OR REPLACE FUNCTION generate_cref() RETURNS TRIGGER AS $$
DECLARE
    new_cref VARCHAR(10);
BEGIN
    -- Gerar um número aleatório de 5 dígitos para o CREF
    new_cref := LPAD(FLOOR(RANDOM() * 100000)::TEXT, 5, '0') || '-G/SP';
    
    -- Atribuir o CREF gerado ao campo NEW.cref
    NEW.cref := new_cref;
...
-- Trigger Professor
CREATE TRIGGER set_cref
BEFORE INSERT ON professor
FOR EACH ROW
EXECUTE FUNCTION generate_cref();
...
VALUES
(UPPER('admin'), '00000000000', '1999-01-01', '00000000000', UPPER('-'), 'admin@treinototal.com', 'admin', '2000-01-01', UPPER('-'));
