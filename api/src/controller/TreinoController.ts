...
/** 
 * Interface TreinoDTO
 * Representa os dados de um treino em um sistema de gestão de exercícios físicos, incluindo informações sobre o aluno, 
 * o professor responsável e os exercícios a serem realizados.
 * Utilizada para padronizar o recebimento de dados de treinos em requisições HTTP.
 * 
 * Atributos:
 * - id_aluno: number - Identificador único do aluno ao qual o treino está associado.
 * - id_professor: number - Identificador único do professor responsável pelo treino do aluno.
 * - exercicios: Array<{ 
 *     id_aparelho: number; - Identificador do aparelho utilizado no exercício.
 *     repeticoes: string; - Número de repetições a serem realizadas durante o exercício.
 *     carga: string; - Peso a ser utilizado no exercício, representado como uma string para permitir unidades como 'kg'.
 *     series: string; - Número de séries a serem completadas para o exercício, representado como uma string.
 * }>
 */
interface TreinoDTO {
    idAluno: number;
 ...
 */
class TreinoController extends Treino {
    ...
    public async novo(req: Request, res: Response): Promise<Response> {
        try {
            ...
            const treinoRecebidoCliente: TreinoDTO = req.body;

            // Extraindo os IDs dos exercícios
            const exerciciosDetalhados = treinoRecebidoCliente.exercicios.map((exercicio: { idAparelho: number, repeticoes: string, carga: string, series: string }) => ({
                id_exercicio: exercicio.idAparelho,
                repeticoes: exercicio.repeticoes,
                carga: exercicio.carga,
                series: exercicio.series
            }));

            const result = await Treino.cadastrarTreino(
                treinoRecebidoCliente.idAluno, 
                treinoRecebidoCliente.idProfessor, 
